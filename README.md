# 🌳 VR English Forest: Interactive Language Learning

**NatureSpeak** is an interactive VR-based educational application designed to help 12-13-year-old children improve their English language skills. By immersing users in a virtual forest filled with animated 3D animals, the application combines fun exploration with educational content to enhance vocabulary, listening, and reading comprehension.

---

## 🎯 Project Goals

- Develop an engaging VR experience for English language learning.
- Encourage vocabulary acquisition, listening, and reading skills through interactive 3D content.
- Create an expandable learning platform with modules based on various natural environments.

---

## 🧭 Features

### 🐾 Virtual Forest Environment
- A fully navigable 3D forest created in Unity.
- Includes natural elements like trees, plants, and ambient sounds.

### 🦌 3D Animal Models & Animations
- Realistic low-poly animals (e.g., deer, bear, bird, rabbit) modeled in Blender.
- Animations include walking, head movements, and idle behaviors.

### 🎧 Interactive Learning
- Touch or approach animals using VR controllers to trigger learning content.
- English descriptions of animals are presented both in text and audio format.

### 🧠 Educational Mini-Games
- Multiple-choice questions and interactive quizzes like "Find the animal" or "Match features".
- Points or rewards awarded for correct answers to boost motivation.

### 🛠️ Modular Design
- The forest is the first learning module.
- Future modules may include environments like the farm, ocean, or space.

---

## 🧑‍🏫 Target Audience

- Children aged 12–13 learning English as a second language.
- Designed with visual and auditory learners in mind.

---

## 🔧 Technologies Used

- **Unity** – Scene building, scripting, and VR integration.
- **Blender** – 3D modeling and animation.
- **C#** – Game logic and interaction scripting.
- **Unity VR SDK** – Integration with devices like Oculus Quest or HTC Vive.

---

## 📦 Project Structure

